...
export default function Header() {
  ...
  const [showSubMenuShop, setShowSubMenuShop] = useState(false);
  ...
  return (
    <>
      ...
      <header className="fixed top-9 max-md:hidden right-0 left-0 bg-black/50 w-[98%] lg:w-[90%] mx-auto px-5 lg:px-10 py-5 h-24 rounded-3xl items-center backdrop-blur-[6px]">
        <div className="flex w-full justify-between items-center">
          <nav className="flex gap-x-5 lg:gap-x-9 items-center">
            ...
            <ul className="flex gap-x-5 lg:gap-x-8 h-full text-gray-300 items-center">
              {menus.map((menu) => (
                <li
                  key={menu.id}
                  className={`${
          ...
          <div className="flex text-orange-200 gap-x-4 lg:gap-x-5 xl:gap-x-10">
            ...
            <a href="#" className="flex items-center gap-x-2.5">
              ...
              <span className="max-xl:hidden tracking-thigtest">
                ورود | ثبت نام
              </span>
            </a>
          </div>
      ...
      {/* Responsive Header */}
      <div className="max-md:flex md:hidden items-center justify-between h-16 w-full bg-white dark:bg-zinc-700 px-4">
        {/* Menu Icon */}
        <FaBars
          onClick={() => {
            setShowMenu(true);
            setShowUserBasket(false);
            setShowSubMenuShop(false);
          }}
        ...
        <div className={`${(showMenu && "subMenuOpen") || "subMenuClose"}`}>
          ...
          <div className="flex items-center justify-between border-b pb-5 mb-6 border-b-gray-100 dark:border-b-white/10">
            <div className="flex items-center gap-x-3.5 ">
              <svg
                className="text-orange-300 w-[41px] h-10"
                viewBox="0 0 41 40"
                fill="currentColor"
              ...
              <svg
                className="text-orange-300 w-[100px] h-10"
                viewBox="0 0 138 55"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
            ...
            <IoCloseSharp
              ...
              className="w-5 h-5 cursor-pointer text-zinc-600 dark:text-white"
            />
          </div>
          {/* Mobail Nav menu */}
          <ul className="flex flex-col gap-4">
            {menus.map((menu) => (
              <li
                ...
                className={`${
                  (menu.status &&
                    "text-orange-300 bg-orange-200/20 rounded-md") ||
                  ""
                }`}
              >
                <a
                  href="#"
                  onClick={() => {
                    if (menu.title === "فروشگاه") {
                      setShowSubMenuShop((prev) => {
                        return !prev;
                      });
                    }
                  }}
                  className={`${
                    (showSubMenuShop &&
                      ...
                      "text-orange-300") ||
                    ""
                  } flex items-center text-zinc-700 ${
                    (showSubMenuShop &&
                      menu.title === "فروشگاه" &&
                ...
                {/* Mobail Nav SubMenu */}
                {menu.subMenu.length > 0 && showSubMenuShop && (
                  <div className="flex flex-col space-y-3 justify-center pr-7 pt-3 pb-3">
                    {menu.subMenu.map((sub) => (
                      <a
                        href="#"
                        key={sub.id}
                        className="group transition-all text-zinc-700 dark:text-white tracking-thigtest"
                      >
                        <LuDot className="hidden p-0 group-active:inline-block w-5 h-5 -mr-2" />
                        {sub.title}
                      </a>
                    ))}
                  </div>
          ...
          {/* Mobail Nav Footer */}
          <div className="flex flex-col items-start justify-center border-t pt-8 mt-8 px-2.5 border-t-gray-100 space-y-6 dark:border-t-white/10 text-orange-300">
        ...
        <div
          className={`${
            (showUserBasket && "userBasketOpen") || "userBasketClose"
          }`}
        >
          {/* Basket Header */}
          <div className="flex items-center w-full pb-5 justify-between border-b border-b-gray-300 dark:border-b-white/10">
            <IoCloseSharp
              onClick={() => {
                setShowUserBasket(false);
              }}
            ...
            />
            <p className="text-zinc-700 dark:text-white">سبد خرید</p>
          </div>
          {/* Basket Body */}
          <div className="w-full h-full overflow-y-scroll">
            {userBasket.map((product) => (
              <div
                key={product.id}
                className="flex py-5 px-4 border-b border-b-gray-300 dark:border-b-white/10"
              >
                <img src={product.img} alt="product" className="w-20 h-20" />
                <div className="tracking-thigtest">
                  <h3 className="line-clamp-2 text-zinc-700 dark:text-white text-sm">
                    {product.title}{" "}
                  </h3>
                  {product.discount && (
                    <span className="text-teal-600 text-xs">
                      {product.discount.toLocaleString()} تومان تخفیف
                    </span>
                  )}
                  <p className="text-zinc-700 dark:text-white text-xl">
                    {(product.discount &&
      ...
      {showMenu && (
        <div
          ...
          className="overlay md:hidden fixed inset-0 w-full h-full z-10 bg-black/40"
        ></div>
      )}
      {showUserBasket && (
        <div
